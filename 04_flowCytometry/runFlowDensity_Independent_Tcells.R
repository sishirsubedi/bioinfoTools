...
gatedFD <- function(flow_data,markers,file_name){
  ...
  if ( (markers[1]=="PE.A") && (markers[2]=="SSC.A") ){
    ...
    # First call to flowDensity
    fd_gate1 <- flowDensity(obj=flow_data, channels=c(markers[1],markers[2]),position=c(TRUE, FALSE), percentile=c(0.99, NA))
    # Second call to flowDensity
    fd_gate2 <- flowDensity(obj=fd_gate1, channels=c(markers[1],markers[2]),position=c(TRUE, FALSE), gates=c(FALSE, NA),percentile=c(NA, 0.99))


    fd_result <- flowDensity(flow_data ,channels = c(markers[1],markers[2]), position = c(T,F),gates=c(fd_gate1@gates[1],fd_gate2@gates[2]),ellip.gate = T,scale = .95)
    png(paste(file_name,"_",markers[1],"_",markers[2],"_",markers[3],"_dist_plot.png"))
    plot(flow_data ,fd_result)
    dev.off()
    ...
    fd_result <- flowDensity(flow_data ,channels = c(markers[1],markers[2]), position = c(F,T),gates=c(fd_gate1@gates[1],fd_gate2@gates[2]),ellip.gate = T,scale = .95)
    png(paste(file_name,"_",markers[1],"_",markers[2],"_",markers[3],"_dist_plot.png"))
    plot(flow_data ,fd_result)
    dev.off()
      ...
      # First call to flowDensity
      fd_gate1 <- flowDensity(obj=flow_data, channels=c(markers[1],markers[2]),position=c(FALSE, TRUE), percentile=c(0.95, NA))
      # Second call to flowDensity
