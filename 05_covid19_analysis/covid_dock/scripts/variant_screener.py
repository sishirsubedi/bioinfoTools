...
def genomewide_mutations(df,out_dir,tag):
...
def variant_screener_per_variant_summary(df,db,out_dir,tag):

    log_msg("generating per variant information..")

    df_db = pd.read_csv(db+".csv")

    dfjoin = pd.merge(df,df_db,right_on="MCoVNumber",left_on="strain",how="left",indicator=True)
    ...
    summary_variants = {}
    mcov_variant = []
    for variant in sorted(vois.keys()):
    
        df_variant = dfjoin[dfjoin[variant]>=vois[variant]]
